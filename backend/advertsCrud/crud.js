...
app.put('/:selectedAdvertId/updateAdvertDisplayStatus',async(req,res) => {
    const { selectedAdvertId } = req.params
    try{
        const advertdetail = await Advert.findOne({_id : selectedAdvertId})
        if(advertdetail){
            const check = 'displayStatus' in advertdetail
            if(check === true){
                const newVal = advertdetail.displayStatus === true ? false : true
                advertdetail.displayStatus = newVal
                await advertdetail.save()
            }else{
                Object.assign(advertdetail,{
                    displayStatus:true
                })
                await advertdetail.save()
            }
            res.status(200).json({advertdetail,displayStatus:advertdetail.displayStatus})
        }
    }catch(err){
        res.status(500).json({message:'Internal Server Err'})
    }
