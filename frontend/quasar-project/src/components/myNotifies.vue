      ...
      <q-card-section v-if="this.offerList.length">
        <q-card
          ...
          <q-item-section avatar>
            ...
            <q-item-section>
              {{ data.advertDetail?.activityName? data.advertDetail.activityName : '' }}
              <q-item-section class="text-caption text-grey-5" caption>
                {{ data.advertDetail?.description ? data.advertDetail.description : '' }}
              </q-item-section>
              <q-item-section caption class="text-caption text-grey-5">
                {{ data.offerDetail?.message? data.offerDetail.message : '' }}
              </q-item-section>
            </q-item-section>
            <q-item-section>
              <q-btn 
                v-if="data.advertDetail?._id"
                icon="delete_forever" flat color="red-4" v-on:click="removeNotify(data)"></q-btn>
            </q-item-section>
          </q-item>
        </q-card>
      </q-card-section>
